...
{
	return *(QWORD**)((QWORD)this + UPasta::Offsets::BuffManager::BuffManagerInstance +
		UPasta::Offsets::BuffManager::BuffEntriesEnd);
}
...
bool Object::IsLocalPlayer() { return this->GetHandle() == globals::localPlayer->GetHandle(); }
bool Object::IsAlly() { return this->GetTeam() == globals::localPlayer->GetTeam(); }

bool Object::IsInAARange()
{
	return globals::localPlayer->GetRealAttackRange() >= globals::localPlayer->GetDistanceTo(this);
}
...
Object* ObjectManager::GetObjectInRange(std::string name, float range)
{
	...
	for (auto objToFind : GetMinions())
	{
		...
		if (!objToFind->GetDistanceTo(globals::localPlayer) > range) continue;

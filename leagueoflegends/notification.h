...
public:
	...
	NOTIFY_INLINE void set_title(const char* format, ...) {
		va_list args;
		va_start(args, format);
		NOTIFY_FORMAT(this->set_title, format, args);
		va_end(args);
	}

	NOTIFY_INLINE void set_content(const char* format, ...) {
		va_list args;
		va_start(args, format);
		NOTIFY_FORMAT(this->set_content, format, args);
		va_end(args);
	}
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);
			{
				...
				auto spacing = GetStyle().ItemSpacing;
				auto region = ImGui::GetContentRegionMax();
				
				GetForegroundDrawList()->AddRectFilled(ImVec2(pos.x + pos.x, 20 + region.y - 16), ImVec2(pos.x + pos.x, region.x + region.y - 10), GetColorU32(c::child::outline), 30.f);

			//	GetForegroundDrawList()->AddShadowRect(ImVec2(pos.x + pos.x, 20 + region.y - 16), ImVec2(pos.x + pos.x, region.x + region.y - 10), GetColorU32(text_color), 10.f, ImVec2(0, 0), 30.f);
				GetForegroundDrawList()->AddRectFilled(ImVec2(pos.x + pos.x, 20 + region.y - 16), ImVec2(pos.x + pos.x, region.x + region.y - 10), GetColorU32(text_color), 30.f);
				...
				TextColored(text_color, cheat_name);
				PopFont();

...
namespace packman {
    inline void trigger_veh(uint64_t address) {
        static auto deref_pointer_in_game_space_fn =
            (uint64_t(__fastcall*)(uint64_t))((uint64_t)GetModuleHandleA(NULL) + 0x10818F0); //E8 ? ? ? ? 48 8B C8 E8 ? ? ? ? 80 3B 00  // 48 8B 00 C3
    ...
    inline void decrypt_memory_pages_for_dump() {
        ...
        while (current_chunk < (uint64_t)sysInfo.lpMaximumApplicationAddress) {
            ...
            if (!Syscaller::VirtualQuery((LPVOID)current_chunk, &mbi, sizeof(mbi)))
                break;
